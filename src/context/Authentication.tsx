...
export function Authentication({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    const loginDetailsinLocalStorage = localStorage.getItem("loginDetails") 
    if (loginDetailsinLocalStorage) {
      const parsedLoginDetails=JSON.parse(loginDetailsinLocalStorage)
      setLoginDetails(parsedLoginDetails)
    }
    else {
